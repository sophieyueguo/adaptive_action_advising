...
class CNNRewardModel(nn.Module):
    def __init__(self, action_embed_size=32):
        ...
        self.action_embed = nn.Embedding(3, action_embed_size)
...
def load_data_from_batch(batches, data_type='rewardNN', normalization=1):
    ...
    for batch in batches:
        ...
        for i in range(batch_size): 
            ...
            o = batch['obs'][i][7:].cpu().numpy().reshape(17, 17, 4) #TODO change for other environments
...
def train_model(data_type, train_loader, criterion, 
    ...
    for epoch in range(train_iter): 
        ...
        if epoch % 100 == 0 or epoch == train_iter-1:
...
class WeightedMSELoss(nn.Module):
    def __init__(self, weight_ones=0):
    ...
    def forward(self, input, target):
        ...
        # weights[target > 100] += self.weight_ones
        weights[target < -100] += self.weight_ones
...
# criterion = WeightedMSELoss(10) #best
