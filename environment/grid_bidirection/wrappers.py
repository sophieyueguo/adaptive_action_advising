...
#########################################################
# class KeyPickupBonus(gym.Wrapper):

#     #TODO fix the missing agent direction 
...
#         if front_pos_type == gym_minigrid.minigrid.OBJECT_TO_IDX["key"]:
#             if action == self.env.Actions.pickup and not self.key_picked:
...
#         bonus = 0.0
#         if self.key_picked:
#             if (front_pos[0], front_pos[1]) not in self.key_set:
#                 bonus = 0.5  # Adjust the bonus value as needed
#                 info['bonus'] = True
#                 self.key_set.add((front_pos[0], front_pos[1]))
...
#         bonus = 0.0
#         if self.key_picked:
#             bonus = 0.501  # Adjust the bonus value as needed
#             info['bonus'] = True
...
class UpdateRewardNN(gym.Wrapper):
    ...
    def update_reward_nn(self):
        ...
        if self.does_update:
            train_loader, test_loader, _ = load_data_from_batch(self.batches)
            criterion = WeightedMSELoss(10)
            self.model.train()
